...
export default function Keywords() {
  ...
  const { data: reraKeyWordList, isLoading: loadingReraKeywords } = useQuery({
    queryKey: ['rera-party-keywords', onboardingData.reraIds],
    queryFn: async () => {
      if (!onboardingData.reraIds.length) {
        return [];
      }
      const data = {
        project_ids: JSON.stringify(onboardingData.reraIds),
      };
  ...
  const { data: transactionKeywords, isLoading: loadingTransactionKeywords } =
    useQuery({
      queryKey: ['transaction-keywords', onboardingData.project_id],
      queryFn: async () => {
        if (!onboardingData.project_id) return null;
        const res = await axiosClient.get<{
          ...
          '/onboarding/get-onboarded-project-parties?project_id=' +
